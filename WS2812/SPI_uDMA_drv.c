...
SSI1IntHandler(void)
{
    ...
    //
    ulStatus = MAP_SSIIntStatus(SSI1_BASE, 1);
    ...
    // enabled.
    //
    MAP_SSIIntClear(SSI1_BASE, ulStatus);

    //
    // If the UART0 DMA TX channel is disabled, that means the TX DMA transfer
    // is done.
    //
    if(!MAP_uDMAChannelIsEnabled(UDMA_CHANNEL_SSI1TX))
    {
...
uDMAErrorHandler(void)
{
    ...
    // still debugging our project, we want to infinite loop here so we can
                ...
                uint8_t *pui8DoneVar)
{
    ...
    MAP_SysCtlPeripheralSleepEnable(SYSCTL_PERIPH_SSI1);
    ...
    // source data
    //
    MAP_SSIConfigSetExpClk(SSI1_BASE, SysClockFreq, SSI_FRF_MOTO_MODE_0,
    ...
    //
    MAP_SSIIntEnable(SSI1_BASE,SSI_TXEOT|SSI_TXFF);
